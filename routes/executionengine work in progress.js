...
function finishTestCaseExecution(execution,executionID,testcaseId,testcase){
    ...
    var updateTC = function(){
		...
		var options = {
			host : "http://alm.cat.com", 
			path : "/qcbin/authentication-point/authenticate",
			method: "GET",
			headers : {'Authorization': 'Basic '+new Buffer("banovm" + ':' + "Boschkins29/").toString('base64')}
			};
			...
			ALMConnect(options, 'header','', function(status, data){
				if(status){
					...
					if(data.headers["set-cookie"] != undefined ) {
						createTest(data.headers["set-cookie"]);
					}else{
		...
		function createTest(LWSSO_Cookie){
			/*var queryParam = "{";
			//add Release
			...
			var opt = {
				host: config.host,
				path: "/qcbin/rest/domains/"+"APD"+"/projects/"+"Minestar_QA_Copy"+"/tests",
				method:"POST",
				headers: {"Cookie":LWSSO_Cookie}
			};
			
			var body = '{"Fields":[{"Name":"TS_NAME","values":[{"value":"new test case"}]},{"Name":"TS_TYPE","values":[{"value":"VAPI-XP"}]},]}'
		 
			ALMConnect(opt, 'data',body,function(status,data){
				if(status){
