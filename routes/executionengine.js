...
var almupload = require('../routes/almupload');
...
exports.startexecutionPost = function(req, res){
	...
	var uploadResultsALM = req.body.uploadResultsALM;
	var almTestSetPath = req.body.almTestSetPath;
	var almTestSet = req.body.almTestSet;
	
	projectUser = req.cookies.project;
	var parts = projectUser.split("/");
	project = parts[0];
	
	projectUser = req.cookies.username;
...
function finishTestCaseExecution(execution,executionID,testcaseId,testcase){
    ...
    var updateTC = function(){
		...
		/*unirest.post('http://localhost:8080/bumblebee/updateqc')
		.headers({
			'Accept-Encoding': 'gzip,deflate'
		})
		.query({
			url: 'https://alm.cat.com/qcbin',
			user: 'banovm',
			encrypted_password: 'Xn3A404GommvK3pL7GrYoQ==',
			domain: 'APD',
			project: 'Minestar_QA_Copy',
			testplandirectory: 'Subject\\Testing\\Redwood',
			testname: testcase.testcase.dbTestCase.name,
			mode: 'FULL',
			testset: 'Redwood Tests',
			testlabdirectory: 'Root\\Testing',
			message: 'Step performed',
			status: qcStatus,
		...
		/*common.logger.info('upload results to ALM: '+execution.uploadResultsALM);
		common.logger.info('ALM test set path: '+execution.almTestSetPath);
		common.logger.info('trace: '+ testcase.trace);
		common.logger.info('status: '+ status);
		common.logger.info('result: '+ testcase.result.result);
		common.logger.info('error: '+ testcase.result.error);
		common.logger.info('runtime: '+ (date-testcase.testcase.startDate));
		common.logger.info('date: '+ date);*/
		
		//if execution needs to upload results to ALM
		common.logger.info('ALMUPLOAD boolean: '+execution.uploadResultsALM);
		common.logger.info('ALMUPLOAD almtestset: '+execution.almTestSet);
		if(execution.uploadResultsALM){
			almupload.uploadTestResultsToALM({testSetPath:execution.almTestSetPath,testSet:execution.almTestSet,testInstance:testcase.testcase.dbTestCase.name,status:status,
			duration:(date-testcase.testcase.startDate),execTimeStamp:date,errorTrace:testcase.trace});
		}
    };
...
function agentBaseState(project,executionID,agentHost,port,threadID,callback){
    common.logger.info("__dirname:"+path.resolve(__dirname,"../agent/executionfiles/"+executionID));
	//copyFile(path.join(__dirname, "../public/automationscripts/"+project+"/build/jar"+projectUser+".jar"),path.join(__dirname, "../agent/executionfiles/lib"),callback());
	//sendFileToAgent((path.join(__dirname, "../public/automationscripts/"+project+"/build/jar").toString())+projectUser+".jar","executionfiles/lib",agentHost,port,3,callback());
	//fs.exists(path.resolve(__dirname,"../agent/executionfiles/"+executionID+"/lib"), function (exists) {
	fs.exists(path.resolve(__dirname,"../agent/executionfiles/lib"), function (exists) {
        if(exists == false){
            sendAgentCommand(agentHost,port,{command:"cleanup",executionID:executionID},3,function(message){
				...
				syncFilesWithAgent(agentHost,port,path.join(__dirname, '../public/automationscripts/'+project+"/bin"),"executionfiles/bin",function(error){
					if(error) {callback(error);return}
					//syncFilesWithAgent(agentHost,port,path.join(__dirname, '../launcher'),"executionfiles/"+executionID+"/launcher",function(){
					syncFilesWithAgent(agentHost,port,path.join(__dirname, '../launcher'),"executionfiles/launcher",function(){
						...
						syncFilesWithAgent(agentHost,port,path.join(__dirname, '../public/automationscripts/'+project+"/External Libraries"),"executionfiles/lib",function(){
							...
							syncFilesWithAgent(agentHost,port,os.tmpDir()+"/jar_"+executionID,"executionfiles/lib",function(){
								syncFilesWithAgent(agentHost,port,path.join(__dirname, '../public/automationscripts/'+project+"/build/jar/projectJar"),"executionfiles/lib",function(){
									common.logger.info("synced all files with agent");
									sendAgentCommand(agentHost,port,{command:"start launcher",executionID:executionID,threadID:threadID},3,function(message){
										common.logger.info("inside sendAgentCommand");
										if ((message) && (message.error)){
											common.logger.info("sendAgentCommand error: " & message.error)
											callback(message.error);
										}
										else{
											common.logger.info("sendAgentCommand no error")
											callback();
										}
									});
								})
...
function sendFileToAgent(file,dest,agentHost,port,retryCount,callback){
	...
	common.logger.info("file: "+file);
	common.logger.info("dest: "+dest);
...
function copyFile(source, target, cb) {
	...
	common.logger.info("copyfile source: "+source);
	common.logger.info("copyfile target: "+target);
